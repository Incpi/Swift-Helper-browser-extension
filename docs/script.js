function pc(t) {
  english = `<div class='ui segment'>
  <div class="ui header">States by Color</div>
      <div class="ui fluid tertiary buttons">
        <button class="ui red button">Error</button>
        <button class="ui yellow button">Warning</button>
        <button class="ui green button">Success</button>
        <button class="ui blue button">Ready</button>
      </div>
    </div>
    <div>
    <div class="ui header">How to use Xamta, a Chrome extension for online exam management</div>
      <br>
      If you are looking for a convenient and efficient way to manage your online exams, you might want to try Xamta,
      a Chrome extension that lets you enter marks, import & export, and generate reports for your students. In this
      blog post, I will show you how to install and use Xamta in easy steps.
      <br>
      <div class="ui header">Step 1: Install the extension</div>
      <div class=" ui description">You have two options to install Xamta. You can either use the Chrome Web Store and
        search for Xamta, or you can download it from GitHub and load it as an unpacked extension. To do the latter,
        you need to enable the developer mode in your Chrome settings and then select the folder where you downloaded
        Xamta. Check out Install Tab above.</div>
      <div class="ui header">Step 2: Enable the extension</div>
      <div class="ui description">Once you have installed Xamta, you will see a floating button at the right bottom
        corner of your browser window. <img class='ui rounded centered image' src='Snapshots/floatbutton.png'
          style="width:200px"></img> It has a greyscale icon similar to the extension page icon. When you hover over it,
        you will see an orange icon. Click on the button to initiate process of the extension to get Exam and other
        related details.
      </div>
      <div class="ui header">Step 3: Enter your exam details</div>
      <div class="ui description"> When you click on the extension button, it will open a popup window that contains a
        chat bot interface. Here, you can enter your exam details, such as the exam name, subject, date, and duration.
        If your page has no data of any exam, don't worry.<img class='ui rounded centered image'
          src='Snapshots/blank.png'></img> It's because you haven't set up anything yet. Enter your
        relevant information in the chat bot as you would do on your mobile phone. Once you select an exam and a
        subject in the chat bot, you will see a table with your students' details. Verify and correct them if needed
        in the chat bot. </div>
      <div class="ui header">Step 4: Mark attendance and enter scores</div>
      <div class="ui description"> On the table, you can see your students' names, IDs, and attendance status. You can
        mark them as present or absent for the exam by clicking on the checkboxes. Then, you can enter their scores
        question-wise by typing in the input fields. If you enter a score, the row color will change to blue. This
        indicates that the score is not saved yet.
        <img class='ui rounded centered image' src='Snapshots/exam_snapshot1.png'></img>
      </div>
      <div class="ui header">Step 5: Save your data</div>
      <div class="ui description"> After entering the scores for each student, you need to save them by clicking on
        the save button at the end of each row. The button will be disabled once the data is saved successfully. If it
        fails, you can retry by clicking on the button again.
        <img class='ui rounded centered image' src='Snapshots/markentryblue.png'></img>
      </div>
      <div class="ui header">Step 6: Save all data</div>
      <div class="ui description"> If you have entered scores for multiple students, you can save them all at once by
        clicking on the save all button at the bottom of the table. This will save all the blue rows and disable their
        save buttons. That's it! You have successfully used Xamta to manage your online exam. You can now view and
        export your exam reports by clicking on the report button on the chat bot. You can also access your previous
        exams by selecting them from the drop-down menu on the chat bot. I hope this blog post was helpful for you. If
        you have any questions or feedback about Xamta, feel free to leave a comment below or contact me via email.
      </div>
    </div>`
  gujarati = `<div class='ui segment'>
  <div class="ui header">States by Color</div>
      <div class="ui fluid tertiary buttons">
        <button class="ui red button">Error</button>
        <button class="ui yellow button">Warning</button>
        <button class="ui green button">Success</button>
        <button class="ui blue button">Ready</button>
      </div>
    </div><div>
  <div class="ui header">How to use Xamta, a Chrome extension for online exam management</div>
    <br>
    જો તમે તમારી ઓનલાઈન પરીક્ષાઓનું સંચાલન કરવા માટે અનુકૂળ અને કાર્યક્ષમ રીત શોધી રહ્યા છો, તો તમે Xamta અજમાવી શકો
    છો, એક Chrome એક્સ્ટેંશન જે તમને માર્કસ દાખલ કરવા, આયાત અને નિકાસ કરવા અને તમારા વિદ્યાર્થીઓ માટે રિપોર્ટ્સ
    જનરેટ કરવા દે છે. આ માં બ્લોગ પોસ્ટ, હું તમને બતાવીશ કે Xamta કેવી રીતે ઇન્સ્ટોલ કરવું અને તેનો ઉપયોગ કેવી રીતે
    કરવો. <br>
    <div class="ui header">Step 1: Install the extension</div>
    <div class=" ui description">
      Xamta ઇન્સ્ટોલ કરવા માટે તમારી પાસે બે વિકલ્પો છે. તમે કાં તો Chrome/Egde Web Store નો ઉપયોગ કરી શકો છો અને
      Xamta માટે શોધો, અથવા તમે તેને GitHub પરથી ડાઉનલોડ કરી શકો છો અને તેને અનપેક્ડ એક્સટેન્શન તરીકે લોડ કરી શકો
      છો. આમ કરવા માટે, તમારે તમારી Chrome સેટિંગ્સમાં વિકાસકર્તા મોડને સક્ષમ કરવાની જરૂર છે અને પછી તમે જ્યાં
      ડાઉનલોડ કર્યું છે તે ફોલ્ડર પસંદ કરો. વધુ વિગતો માટે ટેબ ઇન્સ્ટોલ કરવા માટે ક્લિક કરો.
    </div>
    <div class="ui header">Step 2: Enable the extension</div>
    <div class="ui description">એકવાર તમે Xamta ઇન્સ્ટોલ કરી લો, પછી તમે તમારી બ્રાઉઝર વિન્ડોના જમણા નીચેના ખૂણે
      ફ્લોટિંગ બટન જોશો. તેમાં એક્સ્ટેંશન પેજ આઇકન જેવું જ રાખોડી આઇકન છે.
      <img class='ui rounded centered image' src='Snapshots/floatbutton.png' style="width:200px"></img> It has a
      જ્યારે તમે તેના પર તેના પર હોવર કરો, ત્યારે તમને થીમ કલર વિકલ્પ સાથે નારંગી ચિહ્ન દેખાશે. એક્સ્ટેંશનને સક્ષમ
      કરવા
      માટે બટન પર ક્લિક કરો.
    </div>
    <div class="ui header">Step 3: Enter your exam details</div>
    <div class="ui description">જ્યારે તમે એક્સ્ટેંશન બટન પર ક્લિક કરશો, ત્યારે તે એક પોપઅપ વિન્ડો ખોલશે જેમાં ચેટ
      બોટ ઈન્ટરફેસ હશે. અહીં, તમે તમારી પરીક્ષાની વિગતો, જેમ કે પરીક્ષાનું નામ, વિષય, તારીખ અને
      સમયગાળો તપાસો. જો તમારા પૃષ્ઠ પર કોઈપણ પરીક્ષાનો કોઈ ડેટા નથી, તો ચિંતા કરશો નહીં. <img
        class='ui rounded centered image' src='Snapshots/blank.png'></img> તે એટલા માટે છે કારણ કે તમે હજી
      સુધી કંઈપણ સેટ કર્યું નથી. ચેટ બોટમાં તમારી સંબંધિત માહિતી દાખલ કરો જેમ તમે તમારા મોબાઇલ ફોન પર કરો છો. એકવાર
      તમે ચેટ બોટમાં પરીક્ષા અને વિષય પસંદ કરી લો, પછી તમે તમારા વિદ્યાર્થીઓની વિગતો સાથેનું ટેબલ જોશો. ચૅટ બૉટમાં
      જરૂર જણાય તો તેમને ચકાસો અને સુધારો.
    </div>
    <div class="ui header">Step 4: હાજરીને ચિહ્નિત કરો અને સ્કોર્સ દાખલ કરો</div>
    <div class="ui description"> ટેબલ પર, તમે તમારા વિદ્યાર્થીઓના નામ, ID અને હાજરીની સ્થિતિ જોઈ શકો છો. તમે
      ચેકબોક્સ પર ક્લિક કરીને તેમને પરીક્ષા માટે હાજર અથવા ગેરહાજર તરીકે માર્ક કરી શકો છો. પછી, તમે ઇનપુટ ફીલ્ડમાં
      ટાઇપ કરીને તેમના સ્કોર્સ પ્રશ્ન મુજબ દાખલ કરી શકો છો. જો તમે સ્કોર દાખલ કરો છો, તો પંક્તિનો રંગ વાદળી થઈ જશે.
      આ સૂચવે છે કે સ્કોર હજુ સુધી સાચવવામાં આવ્યો નથી.
      <img class='ui rounded centered image' src='Snapshots/exam_snapshot1.png'></img>
    </div>
    <div class="ui header">Step 5: Save your data</div>
    <div class="ui description">દરેક વિદ્યાર્થી માટે સ્કોર્સ દાખલ કર્યા પછી, તમારે દરેક પંક્તિના અંતે સેવ બટન પર
      ક્લિક કરીને તેમને સાચવવાની જરૂર છે. એકવાર ડેટા સફળતાપૂર્વક સેવ થઈ જાય પછી બટન અક્ષમ થઈ જશે. જો તે નિષ્ફળ જાય,
      તો તમે ફરીથી બટન પર ક્લિક કરીને ફરી પ્રયાસ કરી શકો છો.
      <img class='ui rounded centered image' src='Snapshots/markentryblue.png'></img>
    </div>
    <div class="ui header">Step 6: Save all data</div>
    <div class="ui description">જો તમે બહુવિધ વિદ્યાર્થીઓ માટે સ્કોર્સ દાખલ કર્યા છે, તો તમે ટેબલના તળિયે બધા સાચવો
      બટન પર ક્લિક કરીને તે બધાને એકસાથે સાચવી શકો છો. આ બધી વાદળી પંક્તિઓ સાચવશે અને તેમના સેવ બટનોને અક્ષમ કરશે.
      <br>
      બસ આ જ! તમે તમારી ઑનલાઇન પરીક્ષાનું સંચાલન કરવા માટે સફળતાપૂર્વક Xamta નો ઉપયોગ કર્યો છે. હવે તમે ચેટ બોટ પરના
      રિપોર્ટ બટન પર ક્લિક કરીને તમારા પરીક્ષાના અહેવાલો જોઈ અને નિકાસ કરી શકો છો. તમે તમારી અગાઉની પરીક્ષાઓને ચેટ
      બોટ પરના ડ્રોપ-ડાઉન મેનૂમાંથી પસંદ કરીને પણ ઍક્સેસ કરી શકો છો.
      <br> I hope this blog post was helpful for you. If
      you have any questions or feedback about Xamta, feel free to leave a comment below or contact me via email.
    </div>
</div>`
  document.querySelector('#pc').innerHTML = t ? english : gujarati
}

function genimgloop(t) {
  let english = gujarati = "";
  let i = 1
  while (i <= 28) {
    english += `<img class='ui rounded centered image' src='mobile/XAMTA Tutorial-${i}.webp' class='ui fluid inage'></img>`;
    i++
  }
  document.querySelector('#imgloop').innerHTML = english//t ? english : gujarati
}

function how2(t) {
  english = `<div class='ui segment'>
  <div class="ui header">You can either download from EDGE store (Coming soon) or GitHub.</div>
  <div class="ui description">
    How to Install a Chrome Extension from a Zip File or a Github Repository
    <br>
    Chrome extensions are small programs that add functionality to your browser. They can help you customize your
    browsing
    experience, enhance your productivity, or improve your security. However, not all extensions are available on the
    official Chrome Web Store. Sometimes, you may want to install an extension from a zip file or a github repository.
    Here are the steps to do that.
  </div>
  <div class="ui header">Step 1: Download a zip file with the extension</div>
  <div class="ui description">
    If you already have downloaded a file with the plugin - skip this step.
    If you don't have a file, but have a link to a github repository - follow the link, then click the big green "Clone
    or download" button, then click Download ZIP. </div>
  <div class="ui header">Step 2: Extract the contents of the zip file</div>
  <div class="ui description">
    Right click on the downloaded zip file, then click "Extract Here". This will create a folder with the same name as
    the zip file, containing the extension files.
  </div>
  <div class="ui header">Step 3: Open the extension page in google chrome.</div>
  <div class="ui description">
    There are several ways to do that.
    <div class="ui list">
      <a class="item">
        <i class="right triangle icon"></i>
        <div class="content">
          <div class="header">Option 1</div>
          <div class="description">Type chrome://extensions in the url bar and press enter.</div>
        </div>
      </a>
      <a class="item">
        <i class="right triangle icon"></i>
        <div class="content">
          <div class="header">Option 2</div>
          <div class="description">Click on the three dots in the top right of the browser, then click "More tools" then
            click "Extensions".</div>
        </div>
      </a>
    </div>
  </div>
  <div class="ui header">Step 4: Activate developer mode.</div>
  <div class="ui description">
    Turn on the switch on the top right of the page that says "Developer mode". This will enable you to load extensions
    from local files.
  </div>
  <div class="ui header">Step 5: Load unpacked extension.</div>
  <div class="ui description">
    Click on the button on the top left of the page that says "Load unpacked". Then select the folder that contains the
    manifest.json file or file "manifest.json". This is the main file that tells Chrome how to run the extension. You
    should see a new icon on your toolbar indicating that the extension is loaded.
    <br>
    Congratulations! You have successfully installed a Chrome extension from a zip file or a github repository. You can
    now use it as you would any other extension. To uninstall it, simply click on the remove button on the extension
    page.
  </div>
</div>`
  gujarati = `<div class='ui segment'>
  <div class="ui header">EDGE સ્ટોર પરથી (ટૂંક સમયમાં આવી રહ્યું છે) અથવા GitHub પરથી ડાઉનલોડ કરી શકો છો. </div>
  <br>
  <div class="ui description">
    ZIP ફાઇલ અથવા GitHub રિપોઝીટરીમાંથી ક્રોમ એક્સ્ટેંશન કેવી રીતે ઇન્સ્ટોલ કરવુ.
    <br>
    ક્રોમ એક્સ્ટેન્શન એ નાના પ્રોગ્રામ્સ છે જે તમારા બ્રાઉઝરમાં કાર્યક્ષમતા ઉમેરે છે. તેઓ તમને તમારા બ્રાઉઝિંગ
    અનુભવને કસ્ટમાઇઝ કરવામાં, તમારી ઉત્પાદકતા વધારવા અથવા તમારી સુરક્ષા સુધારવામાં મદદ કરી શકે છે. જો કે, અધિકૃત
    ક્રોમ વેબ સ્ટોર પર તમામ એક્સ્ટેન્શન્સ ઉપલબ્ધ નથી. કેટલીકવાર, તમે ZIP ફાઇલ અથવા GitHub રીપોઝીટરીમાંથી
    એક્સ્ટેંશન ઇન્સ્ટોલ કરી શકવુ શકો છો. તે કરવા માટેનાં પગલાં અહીં છે.
  </div>
  <div class="ui header">Step 1: Download a zip file with the extension</div>
  <div class="ui description">
    જો તમે પહેલાથી જ પ્લગઇન સાથે ફાઇલ ડાઉનલોડ કરી હોય તો - આ પગલું અવગણો. જો તમારી પાસે ફાઇલ નથી, પરંતુ તમારી
    પાસે GitHub રીપોઝીટરીની લિંક છે - લિંકને અનુસરો, પછી મોટા લીલા "ક્લોન અથવા ડાઉનલોડ" બટનને ક્લિક કરો, પછી
    ડાઉનલોડ ઝીપ પર ક્લિક કરો.
  </div>
  <div class="ui header">Step 2: Extract the contents of the zip file</div>
  <div class="ui description">
    ડાઉનલોડ કરેલી ZIP ફાઇલ પર જમણું ક્લિક કરો, પછી "અહીં બહાર કાઢો" ક્લિક કરો. આ ZIP ફાઇલના સમાન નામ સાથે એક
    ફોલ્ડર બનાવશે, જેમાં એક્સ્ટેંશન ફાઇલો હશે.</div>
  <div class="ui header">Step 3: Open the extension page in google chrome.</div>
  <div class="ui description">
    આ કરવા માટે 2 રીતો છે.
    <div class="ui list">
      <a class="item">
        <i class="right triangle icon"></i>
        <div class="content">
          <div class="header">Option 1</div>
          <div class="description">URL / Searchbar માં chrome://extensions લખો અને એન્ટર દબાવો. </div>
        </div>
      </a>
      <a class="item">
        <i class="right triangle icon"></i>
        <div class="content">
          <div class="header">Option 2</div>
          <div class="description">બ્રાઉઝરની ઉપર જમણી બાજુએ ત્રણ બિંદુઓ પર ક્લિક કરો, પછી "વધુ સાધનો" ક્લિક કરો
            અને પછી "એક્સ્ટેન્શન્સ" પર ક્લિક કરો.</div>
        </div>
      </a>
    </div>
  </div>
  <div class="ui header">Step 4: Activate developer mode.</div>
  <div class="ui description">
    "Developer mode" કહેતા પૃષ્ઠની ઉપર જમણી બાજુએ સ્વિચ ચાલુ કરો. આ તમને લોકેલ ફાઇલોમાંથી એક્સ્ટેંશન લોડ
    કરવામાં સક્ષમ કરશે. </div>
  <div class="ui header">Step 5: Load unpacked extension.</div>
  <div class="ui description">
    પૃષ્ઠની ઉપર ડાબી બાજુના બટન પર ક્લિક કરો જે કહે છે કે "લોડ અનપેક્ડ". પછી તે ફોલ્ડર પસંદ કરો જેમાં
    manifest.json ફાઇલ અથવા ફાઇલ "manifest.json" હોય. આ મુખ્ય ફાઇલ છે જે Chrome ને એક્સ્ટેંશન કેવી રીતે ચલાવવું
    તે જણાવે છે. તમારે તમારા ટૂલબાર પર એક નવું આયકન જોવું જોઈએ જે દર્શાવે છે કે એક્સ્ટેંશન લોડ થયું છે.
    <br>
    અભિનંદન! તમે ZIP ફાઇલ અથવા GitHub રિપોઝીટરીમાંથી સફળતાપૂર્વક Chrome એક્સ્ટેંશન ઇન્સ્ટોલ કર્યું છે. હવે તમે
    તેનો ઉપયોગ અન્ય કોઈપણ એક્સટેન્શનની જેમ કરી શકો છો. તેને અનઇન્સ્ટોલ કરવા માટે, ફક્ત એક્સ્ટેંશન પૃષ્ઠ પર દૂર
    કરો બટન પર ક્લિક કરો.
  </div>
</div>`
  document.querySelector('#h2use').innerHTML = t ? english : gujarati
}

function init() {
  document.querySelector(".lang").addEventListener('click', () => {
    document.querySelectorAll(".lang .button").forEach(element => {
      element.classList.contains("green") ? element.classList.remove("green") : element.classList.add("green")
    })
    obj = document.querySelector(".lang .green").innerHTML === "ENG"
    how2(obj)
    genimgloop(obj)
    pc(obj)
  });

  obj = document.querySelector(".lang .green").innerHTML === "ENG"
  how2(obj)
  genimgloop(obj)
  pc(obj)
  $('.menu a:not([href]).item').tab()
}
init()